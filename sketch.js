...
const MAX_QUEUE = 130;
const VANISHING_CLEAR_RADIUS = 14;  // centro compositivo suave, no zona prohibida
const OUTLINE_PAD = 3 / 4;
const OUTLINE_ALPHA = 155;
const MAX_BRUSH_REPEATS = 4;
...
const FOCUS_FIELD_SIZE = 170;

// El foco no bloquea todo: solo tiene un núcleo vacío y un anillo de baja densidad.
// Así se lee un centro de remolino sin que parezca un agujero recortado.
const FOCUS_NO_PLACEMENT_RADIUS = 46;
const FOCUS_SOFT_PLACEMENT_RADIUS = 185;
const FOCUS_SOFT_MIN_CHANCE = 0.18;

// Grilla invisible para favorecer zonas todavía poco cubiertas.
// No dibuja nada: solo orienta la aparición de los siguientes brushes.
const COVERAGE_COLS = 9;
const COVERAGE_ROWS = 7;
const COVERAGE_TARGET_CHANCE = 0.68;

// Balance entre cobertura total y lectura espiralada alrededor del foco.
// Más alto: centro/órbita más presentes. Más bajo: campo más uniforme.
const SPIRAL_CENTER_PULL = 0.78;

// Orienta la cara convexa de los brushes hacia el foco.
// Cambiar a 1 si algún set de PNG queda visualmente invertido.
const CONVEX_SIDE_TO_CENTER = -1;

// Los primeros brushes funcionan como base pictórica: grandes y envolventes.
const FOUNDATION_BRUSH_COUNT = 7;
const FOUNDATION_SCALE_START = 1.42;
const FOUNDATION_SCALE_END = 1.16;
...
function initArtwork() {
  ...
  vanishingX = random(width * 0.40, width * 0.60);
  vanishingY = random(height * 0.40, height * 0.60);
...
function resetCoverageGrid() {
  ...
  for (let row = 0; row < COVERAGE_ROWS; row++) {
    ...
    for (let col = 0; col < COVERAGE_COLS; col++) {
      coverageGrid[row][col] = 0;
    }
  }
}

function leastCoveredTarget(stampW) {
  if (!coverageGrid.length) {
    resetCoverageGrid();
  }
  ...
  for (let row = 0; row < COVERAGE_ROWS; row++) {
    for (let col = 0; col < COVERAGE_COLS; col++) {
      let count = coverageGrid[row][col];
  ...
  let picked = candidates[floor(random(candidates.length))];
  return targetFromCell(picked.row, picked.col, stampW);
}

function freeCanvasTarget(stampW) {
  let margin = stampW * 0.18;
...
function targetFromCell(row, col, stampW) {
  let cellW = width / COVERAGE_COLS;
  let cellH = height / COVERAGE_ROWS;
  let margin = stampW * 0.10;

  return {
    x: random(col * cellW - margin, (col + 1) * cellW + margin),
    y: random(row * cellH - margin, (row + 1) * cellH + margin),
  };
}

function markCoverage(fig) {
  if (!coverageGrid.length) {
    resetCoverageGrid();
  }
  ...
  let cellW = width / COVERAGE_COLS;
  let cellH = height / COVERAGE_ROWS;
  let radiusX = max(fig.stampW, fig.stampH) * 0.58;
  let radiusY = radiusX * 0.72;

  for (let row = 0; row < COVERAGE_ROWS; row++) {
    for (let col = 0; col < COVERAGE_COLS; col++) {
      let cx = (col + 0.5) * cellW;
      let cy = (row + 0.5) * cellH;
      let normalized = sq((cx - fig.x) / radiusX) + sq((cy - fig.y) / radiusY);
...
function focusInfluence(x, y) {
  // Devuelve una influencia suave, no una exclusión.
  // El foco organiza levemente el giro, pero ya no deja un centro vacío.
  let theta = atan2(y - vanishingY, x - vanishingX);
  let r = clearRadiusAt(theta);
  let d = dist(x, y, vanishingX, vanishingY);

  return constrain(map(d, r, r + FOCUS_FIELD_SIZE, 0.58, 1), 0.58, 1);
}
...
function radialPlacement(minRatio, maxRatio, drift, brush, stampW) {
  ...
  for (let attempt = 0; attempt < 18; attempt++) {
    ...
    let coverageTarget = random() < COVERAGE_TARGET_CHANCE
      ? leastCoveredTarget(stampW)
      : freeCanvasTarget(stampW);
    let spiralTarget = spiralCoverageTarget(theta, minRatio, maxRatio);
    let pull = SPIRAL_CENTER_PULL + random(-0.08, 0.10);
    let target = {
      x: lerp(coverageTarget.x, spiralTarget.x, constrain(pull, 0.52, 0.88)),
      y: lerp(coverageTarget.y, spiralTarget.y, constrain(pull, 0.52, 0.88)),
    };

    // Deriva lateral: evita que la grilla se vuelva visible.
    let side = random(-drift, drift);
    let x = target.x + cos(theta + HALF_PI) * side;
    let y = target.y + sin(theta + HALF_PI) * side;

    // Un sesgo leve hacia la dirección dominante conserva el flujo de la obra
    // sin generar un centro vacío o una fuga demasiado explícita.
    let flowPush = random(4, 70);
    x += cos(theta) * flowPush;
    y += sin(theta) * flowPush;

    x = constrain(x, -stampW * 0.12, width + stampW * 0.12);
    y = constrain(y, -stampW * 0.12, height + stampW * 0.12);

    let d = dist(x, y, vanishingX, vanishingY);
    theta = atan2(y - vanishingY, x - vanishingX);
    ...
    if (!fallback || candidate.d > fallback.d) {
      fallback = candidate;
    }

    if (focusPlacementAllowed(candidate.d)) {
      return candidate;
    }
  }

  if (fallback.d < FOCUS_NO_PLACEMENT_RADIUS) {
    fallback.d = FOCUS_NO_PLACEMENT_RADIUS + 12;
    fallback.x = vanishingX + cos(fallback.theta) * fallback.d;
    fallback.y = vanishingY + sin(fallback.theta) * fallback.d;
    fallback.focus = focusInfluence(fallback.x, fallback.y);
  }

  return fallback;
}

function focusPlacementAllowed(distanceFromFocus) {
  if (distanceFromFocus < FOCUS_NO_PLACEMENT_RADIUS) {
    return false;
  }

  if (distanceFromFocus >= FOCUS_SOFT_PLACEMENT_RADIUS) {
    return true;
  }

  let chance = map(
    distanceFromFocus,
    FOCUS_NO_PLACEMENT_RADIUS,
    FOCUS_SOFT_PLACEMENT_RADIUS,
    FOCUS_SOFT_MIN_CHANCE,
    1
  );
...
function spiralCoverageTarget(theta, minRatio, maxRatio) {
  let edgeDist = distanceToCanvasEdge(theta, 130);
  let minDist = edgeDist * minRatio * 0.42;
  let maxDist = max(edgeDist * maxRatio, minDist + 80);

  // Alterna entre órbitas cercanas y externas para que el foco exista
  // como remolino, sin transformarse en un agujero fijo.
  let orbitBias = random() < 0.56 ? pow(random(), 1.75) : pow(random(), 0.66);
  let d = lerp(minDist, maxDist, orbitBias);
  let curl = spiralDir * map(d, 0, edgeDist, 0.58, 1.55);
  let a = theta + curl + random(-0.06, 0.06);

  return {
    x: vanishingX + cos(a) * d,
    y: vanishingY + sin(a) * d,
  };
}
...
function spiralAngle(brush, theta, jitter, focus) {
  ...
  // Eje tangencial: arma la órbita del remolino.
  // La constante CONVEX_SIDE_TO_CENTER elige qué lado local del PNG mira hacia
  // el foco, para que la panza/convexidad de la pincelada abrace el centro.
  let tangentWithConvexInward = theta + CONVEX_SIDE_TO_CENTER * HALF_PI;
  let spiralSkew = spiralDir * lerp(0.28, 0.10, focus);

  return tangentWithConvexInward +
    angleOffset +
    spiralSkew +
    random(-jitter * 0.25, jitter * 0.25);
}
...
function modulateStampNearFocus(stampW, focus) {
  // Cerca del foco hay una respiración leve, pero no un vacío.
  return stampW * lerp(0.90, 1.09, pow(focus, 0.65));
}

function alphaNearFocus(focus) {
  // Mantiene opacidad alta para que los brushes cubran el fondo de manera plena.
  return floor(lerp(235, 255, pow(focus, 0.7)));
}
...
function newFigure(soundFamily = null) {
  ...
  if (figIdx <= FOUNDATION_BRUSH_COUNT) {
    if (r < 0.38) return makeSweep(family);
    if (r < 0.68) return makeMass(family);
    return makeRibbon(family);
  }
...
function foundationScale() {
  ...
  let t = (figIdx - 1) / max(1, FOUNDATION_BRUSH_COUNT - 1);
  return lerp(FOUNDATION_SCALE_START, FOUNDATION_SCALE_END, t);
}
...
function makeMass(family) {
  ...
  let stampW = sizeForBrush(brush, 260, 420) * foundationScale();
...
function makeRibbon(family) {
  ...
  let stampW = sizeForBrush(brush, 330, 560) * foundationScale();
...
function makeSweep(family) {
  ...
  let stampW = sizeForBrush(brush, 470, 700) * foundationScale();
...
function makeStem(family) {
  ...
  let stampW = sizeForBrush(brush, 300, 460) * foundationScale();
...
function stamp(label, brush, x, y, angle, stampW, kind, alpha = 255) {
  ...
  return {
    ...
    y,
    stampW,
    stampH: brush.img && brush.img.width > 0
      ? stampW * (brush.img.height / brush.img.width)
      : stampW,
